...
class AudioFeatures():
    ...
    async def _get_melspectrogram(self, x: Union[np.ndarray, List], melspec_transform: Callable = lambda x: x/10 + 2):
        ...
        melspecModelPredict = js.window.melspecModelPredict

        # outputs = self.melspec_model_predict(x)
        js_outputs = await melspecModelPredict(x).to_py()
        
        js.window.console.log("js_outputs", js_outputs)
        
        outputs = js_outputs # .to_py()
    ...
    def _get_embeddings_from_melspec(self, melspec):
        ...
        embeddingModelPredict = js.window.embeddingModelPredict

        if melspec.shape[0] != 1:
            melspec = melspec[None, ]
        embedding = embeddingModelPredict(melspec) # self.embedding_model_predict(melspec)
        return embedding

    async def _get_embeddings(self, x: np.ndarray, window_size: int = 76, step_size: int = 8, **kwargs):
        """Function to compute the embeddings of the provide audio samples."""
        spec = await self._get_melspectrogram(x, **kwargs)
        windows = []
        ...
        embedding = embeddingModelPredict(batch)
        
        return embedding

    async def get_embedding_shape(self, audio_length: float, sr: int = 16000):
